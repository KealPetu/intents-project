...
El usuario abre la Galería nativa del teléfono y selecciona una foto.
Presiona el botón global de "Compartir".
En la lista de aplicaciones compatibles del sistema operativo, debe aparecer el logotipo y nombre de la app creada por el estudiante.
Al seleccionar su aplicación, Android arranca (o maximiza) la Activity de la app y le inyecta un objeto Intent cargado con los datos compartidos.
El framework debe inspeccionar el Intent de arranque. Si detecta que viene con datos, extrae el texto o procesa el Uri de la imagen (por ejemplo, content://media/external/images/media/...) y lo dibuja en pantalla.
3. Tips de Diseño y Mapeo Tecnológico para Estudiantes
Para resolver este desafío, cada estudiante se enfrentará a la forma en que su framework maneja los ciclos de vida nativos:
WebView: El estudiante no puede abrir la cámara ni capturar un Intent entrante directamente desde JS. Debe configurar la Activity de Android (en Kotlin) para que reciba el Intent del sistema. Una vez recibido el texto o el archivo en Kotlin, debe inyectarlo al WebView ejecutando una función global de JavaScript (ej: window.mostrarDatosCompartidos(datos)).
React Native: Para capturar datos entrantes cuando la app es abierta desde afuera, se debe analizar cómo el método de inicio (como Linking o librerías específicas de Intents compartidos) captura el objeto inicial que Android envía a la Activity.
Flutter: El manejo del Activity Result (para la foto) y el procesamiento de los argumentos iniciales recibidos al arrancar por un filtro de Intent requerirán buscar plugins especializados en compartir contenido (sharing extensions) o mapear los métodos en el código de la actividad principal.
Kotlin Multiplatform: El estudiante que trabaje aquí tiene ventaja directa en el lado de Android, ya que puede sobrescribir el método onNewIntent(intent: Intent?) directamente en la MainActivity de su módulo de Android y procesar los extras utilizando las clases estándar del SDK.
NativeScript: Al tener acceso global a las APIs, el estudiante puede consultar en TypeScript el Intent actual mediante application.android.foregroundActivity.getIntent(). Deberá validar los métodos de verificación de tipos para extraer correctamente las cadenas y los flujos de bits de la imagen.
